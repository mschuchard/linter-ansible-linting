  ...
  provideLinter: () => {
    return {
      ...
      lint: (activeEditor) => {
        ...
        for (i = 10; i <= 17; i++) {
          if (atom.config.get("linter-ansible-linting.ANSIBLE00" + i)) {
            skipList += "ANSIBLE00" + i + ",";
          }
        }
        if (skipList != '') {
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            if (lint_matches != null && correct_file_matches != null) {
              toReturn.push({
                type: 'Warning',
                severity: 'warning',
                range: helpers.rangeFromLineNumber(activeEditor, Number.parseInt(lint_matches[1] - 1)),
                text: lint_matches[2],
                filePath: file,
              });
            }
            // check for linting issues in other files
              ...
              toReturn.push({
                type: 'Warning',
                severity: 'warning',
                range: [[Number.parseInt(lint_matches[1]) - 1, 0], [Number.parseInt(lint_matches[1]) - 1, 1]],
                text: lint_matches[2],
                filePath: '/home/matt/git_repos/linter-ansible-linting/spec/fixtures/include_with_issues.yml',
              });
            }
          });
          return toReturn;
