      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, { cwd: projectPath, stream: 'both', timeout: atom.config.get('linter-ansible-linting.timeout') * 1000 }).then(output => {
          ...
          output.stdout.split(/\r?\n/).forEach((line) => {
            ...
            const lintMatchesSyntax = /(.*.ya?ml):(\d+):(\d+):\s([a-z-]+)(?:\[[a-z-]+\]:)?\s(.*)/.exec(line);
            ...
            // check for ansible-lint 5 output linting issues
              ...
              toReturn.push({
                ...
                location: {
                  ...
                  position: [[Number.parseInt(lintMatches[2], 10) - 1, 0], [Number.parseInt(lintMatches[2], 10) - 1, 1]],
                },
              });
            }
            // check for ansible-lint 5 syntax issues with info
              ...
              toReturn.push({
                ...
                location: {
                  ...
                  position: [[Number.parseInt(lintMatchesSyntax[2], 10) - 1, Number.parseInt(lintMatchesSyntax[3], 10) - 1], [Number.parseInt(lintMatchesSyntax[2], 10) - 1, Number.parseInt(lintMatchesSyntax[3], 10)]],
                },
              });
            }
