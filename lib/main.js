...
export default {
  config: {
    ...
    lintProject: {
      title: 'Lint Project',
      type: 'boolean',
      description: 'Lint entire project if open file is not a playbook (useful if your projects are playbook collections or roles)',
      default: false,
    },
    versionFive: {
  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        // lint file if playbook, else lint project (assuming project is role)
        if ((lintProject) && (!(/hosts:/.exec(activeEditor.getText()))))
