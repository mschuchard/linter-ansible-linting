...
export default {
  config: {
    ...
    fiveBeta: {
      title: 'Ansible-Lint 5 Beta',
      type: 'boolean',
      description: 'Enable beta support for ansible-lint 5.',
      default: false,
    }
  },
  ...
  activate() {
    ...
    helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), ['-T']).then(output => {
      if (/no-loop-var-prefix/.exec(output))
        // ansible-lint >= 5
        atom.config.set('linter-ansible-linting.fiveBeta', true)
      else {
        // ansible-lint < 5
        atom.config.set('linter-ansible-linting.fiveBeta', false)
  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (atom.config.get('linter-ansible-linting.fiveBeta'))
          args.push('--offline');
        ...
        return helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, {cwd: dir, stream: 'both', ignoreExitCode: true, timeout: atom.config.get('linter-ansible-linting.timeout') * 1000}).then(output => {
          output.stdout.split(/\r?\n/).forEach((line) => {
            ...
            // check for linting issues in other files
              ...
              toReturn.push({
                ...
                location: {
                  ...
                  // prepend dir to file
                  file: dir + require('path').sep + lint_matches[1],
                  position: [[Number.parseInt(lint_matches[2]) - 1, 0], [Number.parseInt(lint_matches[2]) - 1, 1]],
                },
              });
            }
            // check for ansible-lint 5 output linting issues
            else if (atom.config.get('linter-ansible-linting.fiveBeta') && (lint_matches_new != null)) {
              toReturn.push({
                severity: 'warning',
                ...
                location: {
                  // ansible-lint plus atom-linter combined now lose pathing info for this kind of parsing
                  // prepend dir to file
                  file: dir + require('path').sep + lint_matches_new[1],
            ...
            else if (atom.config.get('linter-ansible-linting.fiveBeta') && (lint_matches_syntax != null)) {
              toReturn.push({
                severity: 'error',
                ...
                location: {
                  // ansible-lint plus atom-linter combined now lose pathing info for this kind of parsing
                  // prepend dir to file
                  file: dir + require('path').sep + lint_matches_syntax[1],
