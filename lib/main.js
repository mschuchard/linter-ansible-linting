  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        const lint_regex_syntax_unknown = /syntax-check: Ansible syntax check failed/
        ...
        return helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, {cwd: dir, stream: 'both', ignoreExitCode: true, timeout: atom.config.get('linter-ansible-linting.timeout') * 1000}).then(output => {
          output.stdout.split(/\r?\n/).forEach((line) => {
            ...
            // display ansible-lint 5 syntax issue with unknwown info
            else if (atom.config.get('linter-ansible-linting.versionFive') && (syntaxUnknown == true) && (lint_matches_syntax_unknown_message != null)) {
              syntaxUnknown = false
              toReturn.push({
                severity: 'error',
                excerpt: lint_matches_unknown_message[1],
                location: {
                  file: file,
                  position: [[0, 0], [0, 1]],
                },
