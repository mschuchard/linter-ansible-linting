...
export default {
  ...
  activate: () => {
    ...
    helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), ['--version']).then(output => {
      ...
      //TODO: eventually change to 2.5 for improved error checking and -R
  ...
  provideLinter: () => {
    return {
      ...
      lint: (activeEditor) => {
        ...
        //TODO: flip  to -R and remove useRulesDirs bool when min becomes 2.5
        ...
        .catch(error => {
          ...
          const matches_three = /(raise Ansible(Parser)?Error|Syntax Error while loading YAML)/.exec(error.message);
          const matches_five = /Line number.*greater than maximum line/.exec(error.message);
          if (matches != null) {
            toReturn.push({
              type: 'Error',
              range: 1,
