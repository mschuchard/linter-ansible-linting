...
export default {
  ...
  activate() {
    ...
    this.helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), ['--version']).then(output => {
    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (atom.config.get('linter-ansible-linting.blacklist') !== '') {
          const blacklist = new RegExp(atom.config.get('linter-ansible-linting.blacklist'));
        ...
        let projectPath = atom.project.relativizePath(dir)[0];

        // pep8 output and no color
        const args = ['-p', '--nocolor', '--offline'];
        ...
        if (atom.config.get('linter-ansible-linting.useProjectConfig')) {
          ...
          if (atom.config.get('linter-ansible-linting.rulesDirs')[0] !== '') {
            ...
            for (let i = 0; i < atom.config.get('linter-ansible-linting.rulesDirs').length; i++)
          ...
          if (atom.config.get('linter-ansible-linting.excludeDirs')[0] !== '')
            for (let i = 0; i < atom.config.get('linter-ansible-linting.excludeDirs').length; i++)
        ...
        return this.helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, { cwd: projectPath, stream: 'both', timeout: atom.config.get('linter-ansible-linting.timeout') * 1000 }).then(output => {
          // initialize variable for linter return here for either linter output or errors
          const toReturn = [];

          output.stdout.split(/\r?\n/).forEach((line) => {
            // setup matchers
            const lintMatches = /(.*.ya?ml):(\d+):(\d+)?:?\s([a-z-\[\]]+):?\s(.*)/.exec(line);
            ...
            // check for ansible-lint 5 output linting issues
              ...
              // determine severity and col info
              const severity = /syntax-check/.exec(lintMatches[4]) == null ? 'warning' : 'error';
              const col = lintMatches[3] == null ? 1 : Number.parseInt(lintMatches[3], 10);

              toReturn.push({
                severity,
                excerpt: `${lintMatches[4]}: ${lintMatches[5]}`,
                location: {
                  // ansible-lint plus atom-linter combined now lose pathing info for this kind of parsing
                  // prepend dir to file
                  file: projectPath + lintMatches[1],
                  position: [[Number.parseInt(lintMatches[2], 10) - 1, col - 1], [Number.parseInt(lintMatches[2], 10) - 1, col]],
                },
              });
            }
