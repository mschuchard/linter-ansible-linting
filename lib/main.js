      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, {cwd: projectPath, stream: 'both', timeout: atom.config.get('linter-ansible-linting.timeout') * 1000}).then(output => {
          output.stdout.split(/\r?\n/).forEach((line) => {
            // setup matchers
            const lintMatches = /(.*):(\d+):\s\[E\d{3}\]\s(.*)/.exec(line);
            const lintMatchesNew = /(.*.ya?ml):(\d+):\s([a-z-]+)\s(.*)/.exec(line);
            ...
            const lintMatchesSyntaxUnknownMessage = /ERROR! (.*)/.exec(line);
            ...
            else if (lintMatchesSyntaxUnknown != null)
              syntaxUnknown = true
            // display ansible-lint 5 syntax issue with unknown info
            else if (syntaxUnknown && (lintMatchesSyntaxUnknownMessage != null)) {
              syntaxUnknown = false
              toReturn.push({
                severity: 'error',
