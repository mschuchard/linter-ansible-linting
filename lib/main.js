...
//TODO: add atom testrunner(jasmine+travisci); subvert include issue; ansible-lint executes on playbooks; double-check line var for checks if there is no 'name' before module
...
export default {
  config: {
    ...
    ANSIBLE0008: {
      title: 'Instead of sudo/sudo_user, use become/become_user.',
      type: 'boolean',
      default: false
    },

    ANSIBLE0009: {
      title: 'Numeric file permissions without leading zero can behavein unexpected ways.',
      type: 'boolean',
      default: false
    }
  },
      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-ansible-linting.ansibleLintExecutablePath'), args, {stream: 'stdout', cwd: path.dirname(file)}).then(output => {
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            rowAdjust = 0;
            if (matches[2] == '0002' || matches[2] == '0003' || matches[2] == '0008') {
              rowAdjust = 1;
            }
            toReturn.push({
              type: 'Warning',
